...
class PocketDimension():
        ...
        for z in range(min_coords[0]-1, max_coords[0]+2):
            for y in range(min_coords[1]-1, max_coords[1]+2):
                for x in range(min_coords[2]-1, max_coords[2]+2):
                    if '{0}_{1}_{2}'.format(x,y,z) not in [cube.id for cube in self.cubes]:
                        self.add_cube(CUBE_STATES['inactive'], (x,y,z))
    ...
    def get_dimensions(self):
        ...
        return ([min(z_values), min(y_values), min(x_values)],
            [max(z_values), max(y_values), max(x_values)])
    ...
    def update_neighbours(self):
        for cube in self.cubes:
            cube.update_neighbours(self.get_neighbours(cube))

    def get_neighbours(self, center_cube):
        neighbours = []
        for cube in [cube for cube in self.cubes if cube.id != center_cube.id]:
            if abs(cube.x - center_cube.x) < 2 and \
                 abs(cube.y - center_cube.y) < 2 and  \
                      abs(cube.z - center_cube.z) < 2:
...
def main():
    ...
    while pocket_dimension.cycle < 6:
        ...
        print('After cycle:', pocket_dimension.cycle)
        print(pocket_dimension)
        


    print('Part 1 answer: ', count_active_cubes(pocket_dimension.cubes))
