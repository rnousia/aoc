...
def run_computer(sequence):
    ...
    opcode = None
    for count, integer in enumerate(integers):
        if count % 4 == 0:
            opcode = integer
            if opcode == 99:
                break
            first = integers[count+1]
            second = integers[count+2]
            result = integers[count+3]
            if opcode == 1:
                integers[result] = integers[first] + integers[second]
            elif opcode == 2:
                integers[result] = integers[first] * integers[second]
            else:
...
def main():
    ...
    for noun in range(100):
        for verb in range(100):
            code_sequence[1] = noun
            code_sequence[2] = verb
            if run_computer(code_sequence)[0] == 19690720:
                print(100 * noun + verb)
                break
